...
class WishlistToggle(APIView):
    ...
    def put(self, request, pk, room_pk):
        ...
        if wishlist.rooms.filter(pk=room.pk).exists():  # if the room is already in the user's wishlist
            wishlist.rooms.remove(room)  # remove the room from the wishlist
        else:  # if it is not in the wishlist
            wishlist.rooms.add(room)  # add the room to the wishlist
        return Response(status=HTTP_200_OK)
